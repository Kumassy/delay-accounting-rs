...
fn update_metrics(stats: &Taskstats) {
    ...
    VERSION.with_label_values(&[&pid.to_string(), &comm]).set(stats.version as i64);
    AT_EXITCODE.with_label_values(&[&pid.to_string(), &comm]).set(stats.ac_exitcode as i64);

    AC_FLAG.with_label_values(&[&pid.to_string(), &comm]).set(stats.ac_flag as i64);
    AC_NICE.with_label_values(&[&pid.to_string(), &comm]).set(stats.ac_nice as i64);

    CPU_COUNT.with_label_values(&[&pid.to_string(), &comm]).set(stats.cpu_count as i64);
    CPU_DELAY_TOTAL.with_label_values(&[&pid.to_string(), &comm]).set(stats.cpu_delay_total as i64);

    BLKIO_COUNT.with_label_values(&[&pid.to_string(), &comm]).set(stats.blkio_count as i64);
    BLKIO_DELAY_TOTAL.with_label_values(&[&pid.to_string(), &comm]).set(stats.blkio_delay_total as i64);

    SWAPIN_COUNT.with_label_values(&[&pid.to_string(), &comm]).set(stats.swapin_count as i64);
    SWAPIN_DELAY_TOTAL.with_label_values(&[&pid.to_string(), &comm]).set(stats.swapin_delay_total as i64);

    CPU_RUN_REAL_TOTAL.with_label_values(&[&pid.to_string(), &comm]).set(stats.cpu_run_real_total as i64);
    CPU_RUN_VIRTUAL_TOTAL.with_label_values(&[&pid.to_string(), &comm]).set(stats.cpu_run_virtual_total as i64);

    AC_SCHED.with_label_values(&[&pid.to_string(), &comm]).set(stats.ac_sched as i64);
    AC_UID.with_label_values(&[&pid.to_string(), &comm]).set(stats.ac_uid as i64);
    AC_GID.with_label_values(&[&pid.to_string(), &comm]).set(stats.ac_gid as i64);
    AC_PID.with_label_values(&[&pid.to_string(), &comm]).set(stats.ac_pid as i64);
    AC_PPID.with_label_values(&[&pid.to_string(), &comm]).set(stats.ac_ppid as i64);
    AC_BTIME.with_label_values(&[&pid.to_string(), &comm]).set(stats.ac_btime as i64);
    AC_ETIME.with_label_values(&[&pid.to_string(), &comm]).set(stats.ac_etime as i64);
    AC_UTIME.with_label_values(&[&pid.to_string(), &comm]).set(stats.ac_utime as i64);
    AC_STIME.with_label_values(&[&pid.to_string(), &comm]).set(stats.ac_stime as i64);
    AC_MINFLT.with_label_values(&[&pid.to_string(), &comm]).set(stats.ac_minflt as i64);
    AC_MAJFLT.with_label_values(&[&pid.to_string(), &comm]).set(stats.ac_majflt as i64);

    COREMEM.with_label_values(&[&pid.to_string(), &comm]).set(stats.coremem as i64);
    VIRTMEM.with_label_values(&[&pid.to_string(), &comm]).set(stats.virtmem as i64);

    HIWATER_RSS.with_label_values(&[&pid.to_string(), &comm]).set(stats.hiwater_rss as i64);
    HIWATER_VM.with_label_values(&[&pid.to_string(), &comm]).set(stats.hiwater_vm as i64);

    READ_CHAR.with_label_values(&[&pid.to_string(), &comm]).set(stats.read_char as i64);
    WRITE_CHAR.with_label_values(&[&pid.to_string(), &comm]).set(stats.write_char as i64);
    READ_SYSCALLS.with_label_values(&[&pid.to_string(), &comm]).set(stats.read_syscalls as i64);
    WRITE_SYSCALLS.with_label_values(&[&pid.to_string(), &comm]).set(stats.write_syscalls as i64);

    READ_BYTES.with_label_values(&[&pid.to_string(), &comm]).set(stats.read_bytes as i64);
    WRITE_BYTES.with_label_values(&[&pid.to_string(), &comm]).set(stats.write_bytes as i64);
    CANCELLED_WRITE_BYTES.with_label_values(&[&pid.to_string(), &comm]).set(stats.cancelled_write_bytes as i64);

    NVCSW.with_label_values(&[&pid.to_string(), &comm]).set(stats.nvcsw as i64);
    NIVCSW.with_label_values(&[&pid.to_string(), &comm]).set(stats.nivcsw as i64);

    AC_UTIMESCALED.with_label_values(&[&pid.to_string(), &comm]).set(stats.ac_utimescaled as i64);
    AC_STIMESCALED.with_label_values(&[&pid.to_string(), &comm]).set(stats.ac_stimescaled as i64);
    CPU_SCALED_RUN_REAL_TOTAL.with_label_values(&[&pid.to_string(), &comm]).set(stats.cpu_scaled_run_real_total as i64);

    FREEPAGES_COUNT.with_label_values(&[&pid.to_string(), &comm]).set(stats.freepages_count as i64);
    FREEPAGES_DELAY_TOTAL.with_label_values(&[&pid.to_string(), &comm]).set(stats.freepages_delay_total as i64);

    THRASHING_COUNT.with_label_values(&[&pid.to_string(), &comm]).set(stats.thrashing_count as i64);
    THRASHING_DELAY_TOTAL.with_label_values(&[&pid.to_string(), &comm]).set(stats.thrashing_delay_total as i64);

    AC_BTIME64.with_label_values(&[&pid.to_string(), &comm]).set(stats.ac_btime64 as i64);
    
    COMPACT_COUNT.with_label_values(&[&pid.to_string(), &comm]).set(stats.compact_count as i64);
    COMPACT_DELAY_TOTAL.with_label_values(&[&pid.to_string(), &comm]).set(stats.compact_delay_total as i64);

    AC_TGID.with_label_values(&[&pid.to_string(), &comm]).set(stats.ac_tgid as i64);

    AC_TGETIME.with_label_values(&[&pid.to_string(), &comm]).set(stats.ac_tgetime as i64);

    AC_EXE_DEV.with_label_values(&[&pid.to_string(), &comm]).set(stats.ac_exe_dev as i64);
    AC_EXE_INODE.with_label_values(&[&pid.to_string(), &comm]).set(stats.ac_exe_inode as i64);

    WPCOPY_COUNT.with_label_values(&[&pid.to_string(), &comm]).set(stats.wpcopy_count as i64);
    WPCOPY_DELAY_TOTAL.with_label_values(&[&pid.to_string(), &comm]).set(stats.wpcopy_delay_total as i64);
